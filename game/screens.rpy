...
style desk_text is default:
    size 20
    color "#cfcfcf"
    outlines [ (absolute(1), "#000A", absolute(0), absolute(0)) ]

# Jauge réutilisable (CPU, MEM, et toute future jauge)
screen jauge(x, y, value, color_fill):
    fixed:
        xpos x
        ypos y
        xsize 100
        ysize 20
        add Solid("#000000") xsize 100 ysize 20
        add Solid(color_fill) xsize int(100 * value / 100) ysize 20
        
        
# Bureau du heros, afficher en permanence après l'intro.

screen desk_linux():
    
    tag navscreen # permet d'etre afficher en permanence.
    
    imagemap:
        ground "images/wallpaper_background.jpg"
        idle "images/wallpaper_background.jpg"
        hover "images/wallpaper_background.jpg"

    add Solid("#000000") xsize config.screen_width ysize 50
    ...
    fixed:
        ...
        text "| Navigateur " style "desk_text" xpos position_x
        $ position_x += 170
        text "| Explorateur de fichier " style "desk_text" xpos position_x
        $ position_x += 1370
        text "TOR  " style "desk_text" xpos position_x
        $ position_x += 50
        # image "gui/vpn_on.png" xpos position_x
        # $ position_x += 100        
        add "button_green_anim" xpos position_x ypos -1
        $ position_x += 100        
        text "CPU " style "desk_text" xpos position_x
        $ position_x += 100                
        use jauge(position_x, 0, cpu_usage, "#ff9900")
        $ position_x += 100 
        text "MEM " style "desk_text" xpos position_x
        $ position_x += 100                
        use jauge(position_x, 0, mem_usage, "#3366ff")

        text "anon_7741@whos : ~" style "desk_text" xpos config.screen_width xanchor 1.0
 

    add Solid("#000000") xsize config.screen_width ypos 1380
    ...
    timer 0.1 action Function(update_cpu_usage) repeat True
